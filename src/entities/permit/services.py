...
async def get_permit(data: PermitRequestDTO):
    ...
    permit_conditions = await ConfigDAO.get_permit_conditions(data.subject.role, object_attr, data.action)
    permit = await UserDAO.is_reachable(data.subject, data.object, **permit_conditions.model_dump())
    return permit